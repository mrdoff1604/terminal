    ...
    pub async fn new() -> Result<Self, Box<dyn std::error::Error + Send>> {
        ...
        let input_handler: Box<dyn FnMut(&[u8]) -> Vec<u8> + Send> = Box::new(move |data| {
            ...
            let response = match data_str.trim() {
                ...
                "exit" => {
                    // Update alive status
                    *alive_clone.blocking_write() = false;
                    String::from("Exiting...\n")
                },
                _ => format!("Unknown command: {}\n", data_str),
            };
            
            debug!("Mock PTY generated response: {:?}", response);
            response.into_bytes()
        });
...
impl Pty for MockPty {
    async fn write(&mut self, data: &[u8]) -> Result<(), Box<dyn std::error::Error + Send>> {
        debug!("Mock PTY writing {} bytes", data.len());
        ...
        // Add the response to the shared buffer using async write lock
        let mut buffer = self.buffer.write().await;
        buffer.extend_from_slice(&response);
        
        debug!("Mock PTY write completed, generated {} bytes response", response.len());
        Ok(())
    }
    
    async fn read(&mut self, buffer: &mut [u8]) -> Result<usize, Box<dyn std::error::Error + Send>> {
        // Only log read operation when there's actual data to read
        // debug!("Mock PTY read operation started, buffer size: {}", buffer.len());
        
        // Use async write lock to read and modify the buffer
        let mut internal_buffer = self.buffer.write().await;
        
        if internal_buffer.is_empty() {
            // No data available, return 0 to simulate non-blocking behavior
            // Skip logging to avoid excessive log output
            return Ok(0);
        }
